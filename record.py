...
class Birthday(Field):
    ...
    def value(self, value: str):
        try:
            ...
            raise ValueError('\033[91mInvalid date format. \
                Correct format: YYYY.MM.DD\033[0m')
...
class Phone(Field):
    ...
    def value(self, value):
        if len(value) != 10 or not value.isdigit():
            raise ValueError('\033[91mThe phone number should be digits only\
                and have 10 symbols.\033[0m')
        self.__value = value
...
class AddressBook(UserDict):
    ...
    def search(self, value: str):
        if len(value) < 3:
            return '\033[91mYou need at least 3 letters to search by name \
                or 3 didgit to search by phone number.\033[0m'
        result = ''
