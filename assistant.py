...
class Assistant(AssistantBot):
    ...
    def handler_user_input(self, commands_menu):
        result = questionary.select('Choose an action:',
    ...
    def table_menu(self, commands, commands_text):
        ...
        for option, values in commands.items():
            if values[-1]:
                color = values[-1]
                table.add_column(option, style=color, width=len(option) + 5,
                                 justify="center")

        row_values = [f"{commands_text}"]
        row_values.extend(
            option for option, values in commands.items() if values[-1])
...
class ContactAssistant(Assistant, AddressBook):
    ...
    def handler(self):
        # запуск файла с сохранеными контактами!!!
    ...
    # поиск рекорд, чтобы избежать ошибку с несущестующим контактом
    @input_error
    def find_record(self):
        ...
        if os.path.isfile(self.phone_book.file):
            self.phone_book.read_from_file()
        print('=' * 150)
        completer = WordCompleter(list(self.phone_book.keys()),
                                  ignore_case=True)
        name = prompt('Enter the name of an existing contact=> ',
                      completer=completer)
    ...
    def search(self):
        ...
        while True:
            print('=' * 150)
            print('\033[38;2;10;235;190mEnter at least 3 letters \
                or numbers to search or press ENTER to exit.\033[0m')
    ...
    def show_all(self):
        while True:
            ...
            table.add_column("Days to birthday", style="yellow",
                             justify="center")
            print('=' * 150)
            print('\033[38;2;10;235;190mEnter how many records to display \
                or press ENTER to skip.\033[0m')
                    ...
                    for name, record in self.phone_book.data.items():
                        ...
                        if iteration_count % item_number == 0:
                            ...
                            table.add_column("Address",
                                             style="yellow",
                                             justify="center")
                            table.add_column("Days to birthday",
                                             style="yellow",
                                             justify="center")
                    if metka == 1:
                        self.console.print(table)
...
class AddAssistant(ContactAssistant):
    ...
    def add_address(self, record: Record):
        print('\033[38;2;10;235;190mEnter your address or press ENTER to skip.\
            \033[0m')
        address = input('Enter address=> ')
        if address:
            record.add_address(address)
            self.save_record(record)
            print(f'\033[38;2;10;235;190mThe address {address} has been added.\
                \033[0m')
            return
        else:
            return
    ...
    def add_birthday(self, record: Record):
        while True:
            try:
                print('\033[38;2;10;235;190mEnter the date of birth or \
                ...
                if birth:
                    ...
                    self.save_record(record)
                    print(f'\033[38;2;10;235;190mThe date of birth {birth} \
                        has been added.\033[0m')
                    return
                else:
                    return
    ...
    def add_email(self, record: Record):
        while True:
            try:
                print('\033[38;2;10;235;190mEnter the email \
                    or press ENTER to skip.\033[0m')
                email = input('Enter email=> ')
                if email:
                    record.add_email(email)
                    self.save_record(record)
                    print(f'\033[38;2;10;235;190mThe email {email}\
                        has been added.\033[0m')
                    return
                else:
                    return
    ...
    def add_phone(self, record: Record):
        ...
        while True:
            try:
                print(f'\033[38;2;10;235;190mPlease enter the Phone Number \
                    {count_phone}, or press ENTER to skip.\033[0m')
                phone = input('Enter phone=> ')
                if phone:
                    record.add_phone(phone)
                    self.save_record(record)
                    print(f'\033[38;2;10;235;190mThe phone number \
                        {phone} has been added.\033[0m')
                    count_phone += 1
                else:
                    return
            except ValueError as e:
    ...
    def edit_name(self):
        while True:
            ...
            if new_name:
                ...
                self.phone_book.data[new_name] = self.phone_book.data.pop(
                    old_name)
                record.name.value = new_name
                self.save_record(record)
                print(f'\033[38;2;10;235;190mName changed successfully from  \
...
class DeleteAssistant(ContactAssistant):
    ...
    def delete_contact(self):
        while True:
            ...
            if not record:
                ...
                return
            self.console.print(self.table_print(record))
            print('\033[91mDo you really want to delete this contact? \
                Please enter the number: 1.YES or press ENTER to skip.\033[0m')
            res = input('Enter your text=>  ').lower()
            if res in ('1', 'yes'):
                self.phone_book.delete(record.name.value)
                print(f'\033[38;2;10;235;190mThe contact {record.name.value} \
                    was removed.\033[0m')
                self.phone_book.write_to_file()
                return
            else:
    ...
    def delete_phone(self):
        while True:
            ...
            print(f'\033[38;2;10;235;190m\
                The phone number {phone} was removed.\033[0m')
            self.save_record(record)
            print('\033[38;2;10;235;190mYou changed the contact:\n.\033[0m')
            self.console.print(self.table_print(record))
            return
...
class BirthAssistant(ContactAssistant):
    ...
    def birthdays_for_date_menu(self):
        table = Table(title="Birthdays information",
                      style="cyan",
                      title_style="bold magenta",
                      width=100)
        table.add_column("Name", style="red", justify="center")
    ...
    # виводити список контактів, у яких день народження через задану к-ть днів
        ...
        for n, rec in self.phone_book.data.items():
            ...
            if rec.birthday:
                ...
                if date_today <= birth <= date_value:
                    contact_birth.append(f'{name}; \
                        {rec.birthday.value}; {rec.days_to_birthday()}')

        if len(contact_birth) == 0:
            print('\033[38;2;10;235;190m\
                No Birthday during this period.\033[0m')
            return None
        return contact_birth
    ...
    def birthday_in_given_days_menu(self):
        ...
        table.add_column("Days to birthday", style="bold blue",
                         justify="center")
        if not self.phone_book:
            print('\033[91mNo contacts.\033[0m')
            return
        while True:
            print('\033[38;2;10;235;190mEnter the required number of days \
                (no more than one year) or press ENTER to skip.\033[0m')
            item_number = input('\033[38;2;10;235;190m\
            ...
            if item_number:
                if item_number.isdigit() and int(item_number) < 365:
                    ...
                    # Введены буквы
                    print(f'\033[91mYou entered a number greater \
                        than one year: {item_number}\033[0m')
            return
...
class NotesAssistant(Assistant):
    ...
    def table_print_note(self):
        table = Table(title="Note Information",
                      style="cyan",
                      title_style="bold magenta",
                      width=100)
    ...
    def delete_note_by_index(self, tag):
        ...
        if len(self.notes) == initial_len:
            ...
            print(f'\033[92mNote with tag "{tag}" deleted successfully.\
                \033[0m')
    ...
    def note_search_menu(self):
        ...
        if sorted_notes:
            print(f'\033[92mFound and Sorted Notes with Tag \
    "{tag_to_search}":\033[0m')
...
class GoodiesAssistant(Assistant):
    ...
    def handler(self):
        anecdot_menu = AnecdotesAssistant()
        birth_menu = BirthAssistant()
        exit_menu = ExitAssistant()
        commands_text = "How can I assist you? Please select an option:"
        commands_menu = {
            'WEATHER': [self.weather_menu, "blue"],
            'ANECDOTES': [anecdot_menu.handler, "blue"],
    ...
    def get_weather(self, api_key, city):
        ...
        if response.status_code == 200:
            ...
            print(f"\033[38;2;10;235;190mThe weather in {city}: \
                {temperature}°C, {description}\033[0m")
        else:
            print(f"\033[91mFailed to get weather. Code status: \
